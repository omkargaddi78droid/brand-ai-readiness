...
| CQ-10 | Temporal freshness & contradiction | Stale "updated" dates; page-to-page contradictions on the same fact | — | None | ✘ | IMPLEMENTED (cycle 23) — both halves now ship. Cross-page fact-collision half (Phase 4 B4): `content-quality-audit` `--sample-file` mode, agent-judged — within CQ-13's own same-template strata, extracts "Label: value" lines with a typed value (price/date/count) and narrows to a label stated on 3+ pages with 2+ distinct values; agent judges genuine collision vs. expected per-item variation (the dominant false positive) against references/content-judgement-rubric.md §CQ-10. Single-page freshness half (Phase 2 B7, added after Phase 4): `--url` mode, entirely script-decided — compares a visible "Last updated"/"As of" string (or JSON-LD `dateModified`) against the HTTP `Last-Modified` header from `shared/page_fetch.fetch_page`; fires only when the claimed date is 30+ days newer than what `Last-Modified` supports, and refuses to fire at all when `Last-Modified` sits within an hour of the fetch time — the dominant false positive, a CDN stamping `Last-Modified` with serve time rather than true edit time, is indistinguishable from a genuine recent edit within a single fetch. Capped at Medium confidence and severity |
...
| INF-10 | Budget & timeout governor | Coverage modes; per-stage time caps; degrade to `unknown` rather than overrun 5 minutes | PARTIALLY_COVERED — `shared/budget.py` (D4, cycle 23): `StageBudget` monotonic-clock cap, `coverage_manifest` attachable via `finding_contract.build_report`'s optional `coverage` param. Now wired into all four `--sample-file` fetch loops (`entity-audit`'s ENT-07/ENT-08, `content-quality-audit`'s CQ-13/CQ-10, `citability-audit`'s CIT-08/CIT-09, `engagement-audit`'s EN-04/EN-08/EN-11) at a 90s cap per invocation: on expiry, fetching stops, already-fetched pages still get findings computed over them, every un-fetched page gets its own `unknown_checks` entry naming the cap, and each dispatcher's `coverage` field is merged by `compose_report.py` into the final report's `coverage.stages` list. The in-memory O(n²) comparison stages (e.g. B6/CQ-13's near-duplicate shingle comparisons) stay uncapped — fast enough at this project's realistic sample sizes (~30 pages) that a wall-clock cap adds no safety, per `02-project-plan.md` Part 1's own rejection of `datasketch` on the same grounds. Not yet consumed by any single-page (once-per-page) skill invocation, only the multi-page `--sample-file` modes |
